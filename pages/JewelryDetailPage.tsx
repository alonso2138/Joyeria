...
const JewelryDetailPage: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="h-screen flex items-center justify-center"><Spinner text={config?.uiLabels?.loadingItems || "Cargando..."} /></div>;
  }

  if (!item) {
    return <div className="h-screen flex items-center justify-center text-2xl font-serif">{config?.uiLabels?.noItemsFound || "No encontrada."}</div>;
  }
