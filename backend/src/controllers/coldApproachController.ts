    ...
    try {
        ...
        for (let i = 0; i < leads.length; i++) {
            try {
                ...
                const subject = resolveSpintax((config as any)?.marketing?.emailSubject || `{Una idea para vuestra web|${leads[i].nombre}, rápido|Pregunta rápida}`);
                const body = resolveSpintax(((config as any)?.marketing?.emailBody || `Hola,<br><br>He estado viendo vuestra web y creo que hay una forma bastante clara de mejorar cómo el cliente decide entre varias piezas.<br><br>Básicamente es permitir que las pruebe online en segundos.<br><br>Si te encaja la idea, te preparo una prueba con vuestras propias joyas para que lo veas.<br><br>¿Te interesa que te la monte?<br><br>Alonso<br><br><a href="https://visualizalo.es/#/baja?id=${encodedEmail}" style="color: #666; font-size: 10px; text-decoration: none;">Darme de baja</a><br><br><img src="https://api.visualizalo.es/api/trigger/cold-abierto?id=${encodedEmail}" alt="" width="1" height="1" style="display:none!important;min-height:0;height:0;max-height:0;width:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all;" />`).replace('${encodedEmail}', encodedEmail));
