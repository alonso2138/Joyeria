...
const updateLeadEstadoInSheet = async (email: string, newStatus: string) => {
    ...
    console.log(await callSheetsHook({
        action: 'action',
        email: normalized,
        estado: newStatus,
    }));
    ...
    try {
        ...
        for (let i = 0; i < leads.length; i++) {
            try {
                ...
                const subject = resolveSpintax(`{¿Pudiste verlo?|Te escribo rápido|Sobre lo anterior}`);
                const body = resolveSpintax(`Hola,<br><br>Te escribo rápido por si no viste mi mensaje anterior.<br><br>La idea es sencilla: que el cliente pueda probar las joyas online y decidir más fácil.<br><br>Si quieres, te preparo una demo con vuestras piezas y la ves en 1 minuto.<br><br>¿Te la monto?<br><br>Alonso<br><br><a href="https://visualizalo.es/#/baja?id=${encodedEmail}" style="color: #666; font-size: 10px; text-decoration: none;">Darme de baja</a><br><br><img src="https://api.visualizalo.es/api/trigger/follow-abierto?id=${encodedEmail}" alt="" width="1" height="1" style="display:none!important;min-height:0;height:0;max-height:0;width:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all;" />`);
