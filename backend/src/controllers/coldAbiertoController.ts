...
export const coldAbierto = async (req: Request, res: Response) => {
    console.log("Trigger cold abierto");
    try {
        ...
        try {
            const [sheetsResult] = await Promise.all([
                ...
                fetch(SHEETS_HOOK_URL, {
                    ...
                    body: JSON.stringify({
                        action: "trigger",
                        email: ID,
                        estado: "Aperturas de cold"
                    })
                }),
                // Operación de Almacenamiento Local (en paralelo)
                StorageHelper.updateEvents(ID, "cold-approach")
            ]);
            ...
            if (sheetsResult.ok) {
                console.log("Éxito");
                return res.status(200).json({ status: "OK", message: 'Éxito guardando trigger de cold abierto' });
            } else {
